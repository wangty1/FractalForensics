...
class Identity(nn.Module):
    ...
    def forward(self, img_wtm_device):
        return img_wtm_device[0]


class PatchCrop(nn.Module):
    def __init__(self, patch_size=32, wtm_size=8):
        super().__init__()
        self.patch_size = patch_size
        self.wtm_size = wtm_size

    def forward(self, img_wtm_device, x_0, y_0, w, h):
        """
        Crop a patch-aligned region from the image.

        Returns:
            modified_img_wtm: watermarked image with cropped region
            patch_mask: (B, H, W) binary mask — 1 means retained, 0 means cropped
        """
        ...
        x_0_pix = x_0 * self.patch_size
        y_0_pix = y_0 * self.patch_size
        x_1_pix = x_0_pix + w * self.patch_size
        y_1_pix = y_0_pix + h * self.patch_size

        img_wtm[:, :, y_0_pix:y_1_pix, x_0_pix:x_1_pix] = img[:, :, y_0_pix:y_1_pix, x_0_pix:x_1_pix]

        patch_mask = torch.ones((B, self.wtm_size, self.wtm_size), dtype=torch.float32, device=img.device)
        patch_mask[:, y_0:y_0 + h, x_0:x_0 + w] = 0.0

        return img_wtm, patch_mask
...
class Resize(nn.Module):
    ...
    def forward(self, img_wtm_device):
        img = img_wtm_device[1]
        img_wtm = img_wtm_device[0]
        img_wtm = nn.functional.interpolate(img_wtm, scale_factor=(self.ratio, self.ratio), mode=self.interpolation,
                                            recompute_scale_factor=True)
        img_wtm = transforms.Resize((img.shape[-1], img.shape[-2]))(img_wtm)
        return img_wtm
...
class GaussianNoise(nn.Module):
    ...
    def forward(self, img_wtm_device):
        ...
        noise_tensor = torch.randn(img_wtm.shape) * self.std + self.mean
        img_wtm = img_wtm + noise_tensor.to(device)
        return img_wtm
...
class GaussianBlur(nn.Module):
    ...
    def forward(self, img_wtm_device):
        return self.gaussian_filter(img_wtm_device[0])
...
class MedBlur(nn.Module):
    ...
    def forward(self, img_wtm_device):
        return self.middle_filter(img_wtm_device[0])
...
class Combined(nn.Module):
    ...
    def forward(self, img_wtm_device):
        ...
        return self.lst[idx](img_wtm_device)
...
class JpegTest(nn.Module):
    ...
    def forward(self, img_wtm_device):
        ...
        shape = img_wtm.shape
        noised_image = torch.zeros_like(img_wtm)

        for i in range(shape[0]):
            single_image = ((img_wtm[i].clamp(-1, 1).permute(1, 2, 0) + 1) / 2 * 255).to('cpu', torch.uint8).numpy()
            ...
            noised_image[i] = transform(jpeg).unsqueeze(0).to(img_wtm.device)
...
class Jpeg(JpegBasic):
    ...
    def forward(self, img_wtm_device):
        ...
        image_dct, pad_width, pad_height = self.yuv_dct(img_wtm, self.subsample)
